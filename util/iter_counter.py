...
class IterationCounter():
    def __init__(self, opt, dataset_size):
        ...
        self.iter_fid_path = os.path.join(self.opt.checkpoints_dir, self.opt.name, 'fid.txt')
    ...
    def record_current_iter(self, fid=1000):
        np.savetxt(self.iter_record_path, (self.current_epoch, self.epoch_iter),
                   delimiter=',', fmt='%d')
        print('Saved current iteration count at %s.' % self.iter_record_path)
        np.savetxt(self.iter_fid_path, (fid, 0), delimiter=',', fmt='%1.4f')
