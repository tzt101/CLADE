...
class CocoDataset(Pix2pixDataset):
    ...
    def modify_commandline_options(parser, is_train):
        ...
        parser.set_defaults(dataroot='/home/tzt/HairSynthesis/SPADE/datasets/cocostuff/dataset/')
        return parser
    ...
    def get_paths(self, opt):
        ...
        if opt.add_dist:
            dist_dir = os.path.join(root, '%s_dist' % phase)
            dist_paths = make_dataset(dist_dir, recursive=False, read_cache=True)
        else:
            dist_paths = []

        return label_paths, image_paths, instance_paths, dist_paths
